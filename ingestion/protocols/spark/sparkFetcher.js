...
// Spark uses Aave v3–style Pool & liquidityRate
const POOL_ABI = [
  {
...
/**
 * Fetch Spark APY for all assets
 * - Spark is Ethereum-only
 */
async function fetchSparkYields() {
  const results = [];

  for (const asset of Object.values(ASSETS)) {
    const network = resolveNetwork("spark", asset.symbol);
    ...
    // Spark not deployed on this network
    ...
    try {
      ...
      console.error(
        `Spark fetch error [${asset.symbol}]:`,
        err.message
      );
...
module.exports = fetchSparkYields;
